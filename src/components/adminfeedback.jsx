      ...
      {loading ? (
        ...
        <div className="list-group">
          {items.map((fb) => {
            ...
            return (
              <div key={fb._id} className="list-group-item d-flex align-items-center gap-3">
                {fb.avatarUrl ? (
                  <img
                    src={fb.avatarUrl}
                    alt="Avatar"
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 13, color: "#6b6b6b" }}>{new Date(fb.createdAt).toLocaleString()}</div>
                  <div style={{ marginTop: 6, whiteSpace: "pre-wrap" }}>{fb.message}</div>
                </div>
                <button className="btn btn-sm btn-danger" onClick={() => deleteOne(fb._id)}>Delete</button>
              </div>
            );
          })}
        </div>
