...
const VariationSelector = ({ variations, onVariationChange, basePrice }) => {
  ...
  const uniqueColors = [...new Set(variations.filter(v => v.isActive && v.color).map(v => v.color))];
  const uniqueSizes = [...new Set(variations.filter(v => v.isActive && v.size).map(v => v.size))];
  ...
  const isSizeAvailable = (size) => {
    ...
    return variations.some(v => 
      v.isActive && v.color === selectedColor && v.size === size && v.stock > 0
    );
  };
  ...
  const isColorAvailable = (color) => {
    ...
    return variations.some(v => 
      v.isActive && v.size === selectedSize && v.color === color && v.stock > 0
    );
  };
  ...
  return (
    <div className="variation-selector">
        ...
        <div className="variation-group">
          ...
          <div className="color-options">
            {getAvailableColors().map((color) => (
              <button
                key={color}
                className={`color-option ${selectedColor === color ? 'active' : ''} ${!isColorAvailable(color) ? 'disabled' : ''}`}
        ...
        <div className="variation-group">
          ...
          <div className="size-options">
            {getAvailableSizes().map((size) => (
              <button
                key={size}
                className={`size-option ${selectedSize === size ? 'active' : ''} ${!isSizeAvailable(size) ? 'disabled' : ''}`}
      ...
      {selectedVariation && (
        <div className="variation-info">
          <div className="info-item">
            ...
            <span className="info-value price">£{getPrice().toFixed(2)}</span>
          </div>
          <div className="info-item">
      ...
      {(uniqueColors.length > 0 || uniqueSizes.length > 0) && !selectedVariation && (
        <div className="variation-warning">
          Please select {uniqueColors.length > 0 && 'a color'}{uniqueColors.length > 0 && uniqueSizes.length > 0 && ' and '}{uniqueSizes.length > 0 && 'a size'}
        </div>
      )}
    </div>
  );
